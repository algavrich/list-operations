...
def custom_extend(input_list, second_list):
    ...
    """
    # list_to_add = [input_list[-1]]
    for item in second_list:
        custom_append(input_list, item)
        # list_to_add[-1:] = [list_to_add[-1], item]


    # input_list[-1:] = list_to_add

    # [input_list[-1], second_list[0], second_list[1], .... , second_list[-1]]
...
def custom_insert(input_list, index, value):
    ...
    """
    # creating beginning of list to assign instead of slice
    # starting with the value we want to insert
    # value will be at index because it's first in this added list
    add_list = [value]
    # we need to add the slice back into our added list to replace what we took out
    # needs to be added item by item rather than the whole slice
    # so we loop through items in slice, custom appending them to add list
    for item in input_list[index:]:
        # custom append function
        #add_list[-1:] = [add_list[-1], item]
        custom_append(add_list, item)
    # now we have an add list with value followed by the original sliced items
    # we need to assign this list to the original slice in order to replace it
    input_list[index:]= add_list
...
def custom_remove(input_list, value):
    ...
    index = None
    #loop through indeces of input list
    for i in range(len(input_list)):
        #if input list at given index equals the value
        if input_list[i] == value:
            #reassign index to i
            index = i
            #we break because we dont need to find any more indexs of value
            break
...
def custom_pop(input_list):
    ...
    """
    #create var to sava last item in list
    popped = input_list[-1]
    #delete last item
    del input_list[-1]
    #return last item
...
def custom_index(input_list, value):
    ...
    """
    # empty index variable
    index = None
    # loop through range of list
    for i in range(len(input_list)):
        # if item at given range is equal to value
        if input_list[i] == value:
            # store special index in index variable
            index = i
            # break because we don't care about the rest
            break
...
def custom_reverse(input_list):
    ...
    """
    #reassign entire input list to the input list in reverse
...
def custom_contains(input_list, value):
    ...
    for item in input_list:
      ...
      if item == value:
        ...
        #stop looping
        # break
    # should return false if not true    
    return False
